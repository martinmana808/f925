    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are Gary, the advanced AI interface for F925. 
...
---
            ...
            - Contact: f925.limited@gmail.com (or use the form).
            ...
            // Note: I also need to update the `response_format` in the API call to ensure JSON mode if supported by the model, 
            // but Llama 3.3 via Groq supports JSON mode via strict prompting usually, and `json_object` type if available. 
            // I'll check if I need to add `response_format: { type: "json_object" }` in the body. 
            // The plan mentioned "Enforce strict JSON output". I will add `response_format` to the body as well in a separate edit or same edit if contiguous. 
            // Wait, the replaced block is just the system prompt. The body construction is further down.
            // I will do the prompt update first.
          ...messages
        ],
        temperature: 0.7,
        max_tokens: 1024,
